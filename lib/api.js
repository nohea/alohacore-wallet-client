...
var Mnemonic = require('alohacore-mnemonic');
...
API.prototype.getPayPro = function(txp, cb) {
  ...
  if (!txp.payProUrl || this.doNotVerifyPayPro) {
    console.log("API.getPayPro() return nothing");
    return cb();
  }
  
  console.log("API.getPayPro() PayPro.get() txp.payProUrl, self.payProHttp: ", txp.payProUrl, self.payProHttp);
  PayPro.get({
    url: txp.payProUrl,
