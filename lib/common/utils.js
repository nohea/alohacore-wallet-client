...
Utils.verifyMessage = function(text, signature, publicKey) {
  ...
  console.log("  text, signature, publicKey: ", text, signature, publicKey);
  $.checkArgument(text);
...
Utils._tryVerifyMessage = function(hash, sig, publicKeyBuffer) {
  try {
    console.log("_tryVerifyMessage() ");
    var verifyMessageRv = secp256k1.verify(hash, sig, publicKeyBuffer);
    console.log("  secp256k1.verify() verifyMessageRv: ", verifyMessageRv);
    return verifyMessageRv;
  } catch(e) {
    console.log("  secp256k1.verify error: ", JSON.stringify(e));
    return false;
  }
};
